...
void loop() {
  // put your main code here, to run repeatedly:
    ...
    while (1)
    {   
        line[0][( led_pos + 240 )%320] = ( led_last_raw * ( 256 - Alpha ) + red * Alpha )/ 256;
        line[1][( ir_Pos + 240 )%320] =  ( ir_last_raw * ( 256 - Alpha ) + ir * Alpha )/ 256;

        k_number[( led_pos + 240 )%320] = line[0][( led_pos + 240 )%320] - led_last_raw;
    ...
    for (int i = 0; i < 240; i++ )
    {
        led_disdata = map( line[0][( led_pos + i ) %320] , led_max, led_min, 0, 135);
        ir_disdata  = map( line[1][( ir_Pos + i ) %320]  , ir_max, ir_min, 0, 135);
        k_disdata   = map( k_number[( ir_Pos + i ) %320]  , k_number_max, k_number_min, 0, 135);
        {
