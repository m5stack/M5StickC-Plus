 ...
 switch (UWB_MODE)
  {
    case 0:
      ...
      {
        M5.Lcd.fillRect(109, 85, 30, 20, BLACK);//
        DATA = "  0 2F  ";
        timer_flag = 0;
      }
      break;
  ...
  switch (UWB_MODE)
  {
    case 0:
      if (UWB_T_NUMBER > 0 && UWB_T_NUMBER < 5)
      {
        ...
        {
          c--;
          M5.Lcd.drawString(DATA.substring(2  + c * 11, 3 + c * 11), 28, 85 + c * 40, 4 );//Label the serial number 标签序号
          M5.Lcd.drawString(DATA.substring(4  + c * 11, 8  + c * 11), 70, 85 + c * 40, 4); //Distance 距离
        }
        while (b > 0)
        {
          b--;
          M5.Lcd.fillRect(10, 205 - b * 40, 35, 20, BLACK);//
          M5.Lcd.fillRect(65, 205 - b * 40, 60, 20, BLACK);//
        }
      }
      break;
    case 1:
      if (UWB_B_NUMBER == 1)
      {
        M5.Lcd.drawString(DATA, 8, 180, 4);//Prompt information  (Base station 0 only) 提示信息 (仅限基站0)
      }
      break;
  }
}
   ...
   switch (UWB_MODE)
   {
     case 0:
       ...
       if (UWB_T_NUMBER > 0 && UWB_T_NUMBER < 5)   //Tag mode UI display  标签模式UI显示
       {
         ...
         {
           c--;
           M5.Lcd.drawString("B", 10, 85 + c * 40, 4 );
         }
     ...
     case 1:                                              //Base station mode UI display  基站模式UI显示
      M5.Lcd.drawString("Base ", 40, 40, 4);
       M5.Lcd.drawString(String(UWB_B_NUMBER),40, 75, 8 );//UWB_B_NUMBER
       if (UWB_B_NUMBER == 0)
       {
         M5.Lcd.drawString("Waiting...", 10, 180, 4 );
       }
       else
       {
         M5.Lcd.fillRect(10, 180, 135, 30, BLACK);
       }
 ...
 void setup()
{
  ...
  M5.Lcd.drawString("UWB ", 40, 0, 4); //UI
  M5.Lcd.drawString("Tag ", 50, 210, 4);
  M5.Lcd.drawString("Base ", 70, 90, 4);
...
void UWB_clean()
{
  ...
  DATA = "";
  timer_flag = 0;
  M5.Lcd.fillRect(0,20,135,240,BLACK);
}
