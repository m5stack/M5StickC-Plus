...
void setup() {
  ...
  tftSprite.createSprite(240, 135);  //Create a 240x135 canvas. 创建一块160x80的画布
...
void loop() {
  tftSprite.fillSprite(BLACK);  //Fill the canvas with black. 将画布填充为黑色
  tftSprite.setCursor(0, 0, 1); //Set the cursor position and use the 1-point font. 设置光标位置并采用1号字体
  ...
  M5.update();  //Detect whether the keystroke state has changed. 检测按键状态是否发生变化
  delay(100);
}
